...
const sortResult = async(id) =>{
    ...
    sortNum.forEach((sortData) => {
        const sec = parseInt(sortData.others.posted_date); 
    let hours   = Math.floor(sec / 3600);
    let minutes = Math.floor((sec - (hours * 3600)) / 60);
   ...
   //-----------------create cart------------------------------
    ...
    cart.innerHTML = `
        <div class="card  bg-base-100 shadow-xl">
                    <div class = "p-3 relative">
                      <img class = "h-48 mx-auto rounded-md"
                        src="${sortData.thumbnail}"
                      />
                      <p class ="text-xs p-1 rounded-xl absolute bottom-4 right-5 bg-slate-800 text-white">
                      ${sortData.others.posted_date? `${hours} hrs ${minutes} min ago` : ""}
                      </p>
                    </div>
                    <div class="p-3">
                        <div class="flex gap-3">
                            <div class="">
                                <img class="w-10 h-10 rounded-full"
                                  src="${sortData.authors[0]?.profile_picture}"
                                />
                              </div>
                          <h2 class="card-title text-base font-bold">
                            ${sortData.title}
                        ...
                        <div class = "flex gap-2 my-2">
                        <p class="text-sm font-normal">
                        ${sortData.authors[0]?.profile_name}
                        <p>
                        <span>${sortData.authors[0].verified? `<img class="w-5"
                        src="veryfied.png"
                        />` : ""}
                        </span> 
                        </div>                     
                        <p class="text-sm font-normal my-3">total view : ${sortData.others?.views}</p>
                    </div>
                  </div>
        `;
    videoContainer.appendChild(cart);
